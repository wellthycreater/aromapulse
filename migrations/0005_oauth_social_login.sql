...
-- users 테이블에 OAuth 관련 컬럼 추가 (oauth_provider, oauth_id는 이미 존재)
...
-- OAuth 계정 및 세션 테이블은 0004_oauth_authentication.sql에 이미 존재
...
-- 인덱스 추가 (oauth_provider, oauth_id 인덱스는 이미 존재할 수 있음)
-- CREATE INDEX IF NOT EXISTS idx_users_oauth_provider ON users(oauth_provider);
-- CREATE INDEX IF NOT EXISTS idx_users_oauth_id ON users(oauth_id);
-- CREATE INDEX IF NOT EXISTS idx_oauth_accounts_user_id ON oauth_accounts(user_id); -- 이미 존재
-- CREATE INDEX IF NOT EXISTS idx_oauth_accounts_provider ON oauth_accounts(provider); -- 이미 존재
-- CREATE INDEX IF NOT EXISTS idx_sessions_user_id ON sessions(user_id); -- 이미 존재
-- CREATE INDEX IF NOT EXISTS idx_sessions_token ON sessions(token); -- 이미 존재  
-- CREATE INDEX IF NOT EXISTS idx_sessions_expires_at ON sessions(expires_at); -- 이미 존재
