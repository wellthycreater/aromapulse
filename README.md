...
2. **댓글 추가**
   ...
   - (선택) 작성 날짜 지정
   - "댓글 추가" 버튼 클릭
   - ⚠️ **제한사항**: 게시물당 1개의 댓글만 등록 가능 (중복 추가 시 오류 메시지 표시)
...
## 🆕 최신 업데이트 (v1.6.1) - 댓글 관리 개선 (2025-11-15)

### ✅ 추가된 기능

**게시물당 댓글 1개 제한** (중복 방지)
1. **백엔드 검증**
   - 댓글 추가 시 해당 게시물에 이미 댓글이 있는지 자동 확인
   - 이미 댓글이 있으면 `400 Bad Request` 오류 반환
   - 오류 메시지: "이미 댓글이 존재합니다. 게시물당 1개의 댓글만 등록할 수 있습니다."

2. **프론트엔드 처리**
   - 오류 발생 시 사용자에게 명확한 알림 표시
   - 폼은 자동으로 리셋되지 않아 내용 확인 가능
   - 다른 게시물 선택 후 다시 시도 가능

3. **비즈니스 로직**
   - 각 블로그 게시물에는 **대표 댓글 1개**만 유지
   - 댓글 품질 관리 (가장 중요한 댓글만 선별)
...
1. 게시물 A 선택 → 댓글 입력 → 성공 ✅
2. 게시물 A 선택 → 댓글 입력 → 오류 ❌
   → "이미 댓글이 존재합니다. 게시물당 1개의 댓글만 등록할 수 있습니다."
```

### 💡 비즈니스 가치

- **품질 관리**: 각 게시물의 대표 댓글만 선별하여 관리
- **데이터 정확성**: 중복 댓글로 인한 통계 왜곡 방지
- **관리 효율성**: 게시물당 1개 댓글로 간편한 관리

---

**마지막 업데이트**: 2025-11-15  
**버전**: 1.6.1 (게시물당 댓글 1개 제한)  
**상태**: ✅ 개발 및 테스트 완료  
**전략**: 하이브리드 (수동 우선 → 향후 크롤링 확장)
