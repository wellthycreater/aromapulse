...
let allOnedayClasses = [];
let filteredOnedayClasses = [];
let currentPriceFilter = 'all';

// 페이지 로드 시 실행
window.addEventListener('DOMContentLoaded', async () => {
    checkAuth(); // 인증 체크 (접근 제한 없음)
    
    await loadOnedayClasses();
    
    // 페이지 뷰 트래킹
    trackPageView('oneday_classes_list');
});

// 인증 확인 (원데이 클래스는 모든 사용자 접근 가능)
function checkAuth() {
    const token = localStorage.getItem('token');
    const authLink = document.getElementById('auth-link');
    
    if (!token) {
        // 로그인하지 않은 경우 - 로그인 유도 (하지만 페이지는 볼 수 있음)
        authLink.textContent = '로그인';
        authLink.href = '/login';
        return true; // 원데이 클래스는 비로그인 사용자도 볼 수 있음
    }
...
function showAccessDenied() {
    ...
    document.getElementById('oneday-classes-container').style.display = 'none';
            ...
            <div class="max-w-2xl mx-auto text-center py-16">
                <div class="bg-white rounded-xl shadow-lg p-8">
                    ...
                    <p class="text-lg text-gray-600 mb-6">
                        원데이 클래스 서비스는 <strong>B2B 회원 전용</strong>입니다.
                    </p>
...
async function loadOnedayClasses() {
    try {
        ...
        if (response.ok) {
            allOnedayClasses = await response.json();
            filteredOnedayClasses = [...allOnedayClasses];
            renderOnedayClasses();
        } else {
            showEmpty();
        }
...
function renderOnedayClasses() {
    const container = document.getElementById('oneday-classes-container');
    const emptyState = document.getElementById('empty-state');
    
    if (filteredOnedayClasses.length === 0) {
    ...
    container.innerHTML = filteredOnedayClasses.map(workshop => `
        <div class="oneday-class-card bg-white rounded-lg shadow-md overflow-hidden cursor-pointer" onclick="viewOnedayClass(${classItem.id})">
            <!-- Image Placeholder -->
            ...
            <div class="p-4">
                ...
                <!-- Description -->
                <p class="text-sm text-gray-600 mb-3 line-clamp-2">${workshop.description || '원데이 클래스 설명이 없습니다.'}</p>
                ...
                <div class="flex items-center justify-between pt-3 border-t">
                    ...
                    <button onclick="event.stopPropagation(); viewOnedayClass(${classItem.id})" class="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition">
                        자세히 보기
                    </button>
                </div>
            </div>
...
function searchOnedayClasses() {
    ...
    filteredOnedayClasses = allOnedayClasses.filter(workshop => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            setTimeout(searchOnedayClasses, 300);
        });
    }
    
    if (categoryFilter) {
        categoryFilter.addEventListener('change', searchOnedayClasses);
    }
});
