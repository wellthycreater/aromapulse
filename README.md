...
1. **회원 시스템 & OAuth 소셜 로그인**
   ...
   - B2B 사용자 (조향사 / 기업 / 가게)
   - **기업 담당자 역할 관리** ✅ (v1.7.1-v1.7.2)
     - HR팀 담당자 (hr_manager)
     - 조직문화팀 담당자 (culture_team)
     - 복리후생 담당자 (welfare_manager)
     - 워크샵 견적 문의 권한 제어에 활용
   - **회사 규모 관리** ✅ (v1.7.1-v1.7.2)
     - 20인 미만, 20-50인, 50-100인, 100-300인, 300인 이상
     - 워크샵 이용 자격 판단에 활용 (20인 이상만 가능)
   - 이메일/비밀번호 로그인
   - 네이버 소셜 로그인 ✅
   - 카카오 소셜 로그인 ✅
   - 구글 소셜 로그인 ✅
...
7. **관리자 기능**
   ...
   - **클래스 관리 (CRUD)** ✅
   - 패치 신청 관리
   - **고도화된 대시보드 통계 (v1.7.4)** ✅ NEW
     - 주간 방문자 추이 차트 (Chart.js)
     - 디바이스별 통계 (모바일/태블릿/데스크톱)
     - 최근 가입 회원 목록 (상위 5명)
     - 실시간 활동 로그 (로그인/가입)
     - 전일 대비 증감률 표시
     - 방문자 통계 리셋 기능 (테스트용)
...
## 📊 관리자 대시보드 고도화 (NEW v1.7.4) ✅

### 기능 설명
**관리자 대시보드가 데이터 시각화와 실시간 모니터링 기능으로 대폭 강화**되었습니다. 차트, 그래프, 최근 활동 로그를 통해 서비스 운영 현황을 한눈에 파악할 수 있습니다.

### 주요 기능
1. **방문자 통계 개선**
   - 오늘/어제 비교 및 증감률 표시
   - 방문자 통계 리셋 기능 (개발/테스트용)
   - 전일 대비 퍼센트 변화량 시각화
...
### API 엔드포인트
- `GET /api/visitors/stats` - 방문자 통계 (전일 비교 포함)
- `POST /api/visitors/reset` - 방문자 통계 리셋
- `GET /api/admin/dashboard/stats` - 전체 대시보드 통계
- `GET /api/admin/dashboard/recent-users` - 최근 가입 회원
- `GET /api/admin/dashboard/recent-activities` - 최근 활동 로그
- `GET /api/admin/dashboard/device-stats` - 디바이스/브라우저 통계
...
## 🌐 워크샵 둘러보기 기능 (NEW v1.7.2) ✅

### 기능 설명
**모든 사용자가 워크샵 목록을 자유롭게 둘러볼 수 있도록 개선**되었습니다. 로그인 여부와 관계없이 워크샵을 검색하고 상세 정보를 확인할 수 있으며, 견적 문의는 권한이 있는 담당자만 가능합니다.

### 주요 변경사항
- **워크샵 목록 공개**: 모든 사용자 (비로그인 포함)가 워크샵 목록 열람 가능
- **견적 문의 권한 분리**: 
  - **열람**: 누구나 가능 ✅
  - **견적 문의**: B2B 기업 담당자만 가능 (HR팀, 조직문화팀, 복리후생 담당자 / 20인 이상 기업)
- **스마트 권한 체크**: 
  - 워크샵 목록 페이지: 권한 체크 없음
  - 상세 페이지 진입: 권한 체크 없음
  - 견적 문의 제출 시: `checkQuotePermission()` 실행
- **구체적 오류 메시지**: 
  - "로그인이 필요합니다" (비로그인)
  - "B2B 기업 고객만 가능합니다" (B2C 사용자)
  - "20인 이상 규모의 기업만 이용 가능합니다" (소규모 기업)
  - "HR팀, 조직문화팀, 복리후생 담당자만 문의 가능합니다" (일반 직원)

### 테스트 URL
- **워크샵 목록**: https://3000-ixw6l6ek5pa4nw2e7gi09-c07dda5e.sandbox.novita.ai/workshops
- **API 엔드포인트**: https://3000-ixw6l6ek5pa4nw2e7gi09-c07dda5e.sandbox.novita.ai/api/workshops

### 사용자 경험 개선
1. **비로그인 사용자**: 워크샵 둘러보기 → 견적 문의 시도 → 로그인 페이지로 리디렉션
2. **B2C 사용자**: 워크샵 둘러보기 → 견적 문의 시도 → "B2B 전용" 안내 메시지
3. **소규모 기업**: 워크샵 둘러보기 → 견적 문의 시도 → "20인 이상 기업만 가능" 안내
4. **일반 직원**: 워크샵 둘러보기 → 견적 문의 시도 → "담당자 권한 필요" 안내
5. **권한 있는 담당자**: 워크샵 둘러보기 → 견적 문의 제출 → 성공 ✅
...
**Version**: 1.7.1 (B2B-only Workshops Access Control)
**Status**: ✅ Cloudflare Pages 배포 완료
**Production URL**: https://aromapulse.pages.dev (자동 배포)
**Custom Domains**: 
  - https://aromapulse.kr ✅
  - https://www.aromapulse.kr ✅
**Latest Deployment**: https://048d0e34.aromapulse.pages.dev
...
- ✅ 워크샵 메뉴 표시 (B2B 전용)
  - **접근 제한** ✅ (v1.7.1): HR팀, 조직문화팀, 복리후생 담당자만 접근 가능
  - **회사 규모 요구사항**: 20인 이상 기업
  - **워크샵 특징**: B2B 견적 문의 형태, 강사 초빙, 워케이션 옵션
- ✅ 대시보드
- ❌ AI 상담 메뉴 제거 → 블로그 임베드로 통합
...
- 프로필 수정: `/profile` ✅ (NEW - 2025-11-18)
  - **프로필 이미지 업로드**: JPG/PNG 파일 (최대 5MB), Base64 저장
...
- **데이터베이스 스키마**:
  ...
  - `last_browser` - 최근 접속 브라우저 (Chrome, Safari, Firefox, Edge 등)
  - `last_ip` - 최근 접속 IP 주소
  - `last_user_agent` - 최근 User Agent 전체 문자열
- **로그인 시 자동 업데이트**:
  - 모든 로그인 방식에서 자동으로 `users` 테이블 업데이트
  - 로그인 로그 테이블과 별도로 메인 테이블에도 저장
  - 관리자 페이지에서 즉시 확인 가능
...
**3. 마이그레이션** ✅
- **로컬 DB**: 마이그레이션 0036 적용 완료
- **프로덕션 DB**: 수동 적용 필요 (Cloudflare API 키 설정 후)

### 📋 사용 방법

**관리자 회원 관리에서 확인**:
1. 관리자 로그인 후 `/admin-users` 접속
2. 회원 목록에서 각 사용자의 디바이스 정보 확인
3. 디바이스 타입 배지로 모바일/태블릿/PC 구분
4. OS/브라우저 컬럼에서 상세 정보 확인

**데이터 수집**:
- 사용자가 로그인할 때마다 자동으로 최신 디바이스 정보로 업데이트
- 기존 사용자는 null로 표시되다가 다음 로그인 시 자동 업데이트
- 로그인 로그 테이블에도 별도로 히스토리 저장
...
1. **프로덕션 데이터베이스 마이그레이션**:
   - Cloudflare API 키 설정 후 마이그레이션 0036 적용
   - 또는 Cloudflare 대시보드에서 수동 SQL 실행

2. **디바이스 히스토리 추적**:
   - 사용자별 디바이스 변경 히스토리 저장
   - 다중 디바이스 사용 패턴 분석
   - 디바이스 변경 시 보안 알림
...
## 🆕 이전 업데이트 (v1.7.3) - 로그인 추적 및 방문자 리셋 시스템 (2025-11-19)
...
**1. 워크샵 접근 권한 세분화** ✅
- **이전**: 모든 B2B 사용자가 워크샵 페이지 접근 가능
- **현재**: HR팀, 조직문화팀, 복리후생 담당자만 접근 가능
- **접근 조건**:
  - 사용자 유형: B2B
  - 담당자 역할: `hr_manager`, `culture_team`, `welfare_manager`
  - 회사 규모: 20인 ~ 50인 이상 (`20_50`, `50_100`, `100_300`, `300_plus`)
...
- **Hero 섹션 접근 자격 안내**:
  - "워크샵 이용 자격" 박스 추가
  - HR팀, 조직문화팀, 복리후생 담당자 명시
  - 20인 ~ 50인 이상 기업 요구사항 안내
  - 전문 강사 초빙 및 워케이션 옵션 안내

**4. 접근 거부 메시지 상세화** ✅
- **로그인 안 한 경우**: 로그인 페이지로 안내
- **B2C 사용자**: 원데이 클래스 페이지로 안내
- **담당자 권한 없는 B2B**: 원데이 클래스 페이지로 안내
- **20인 미만 기업**: 원데이 클래스 페이지로 안내
- **각 상황별 맞춤 메시지 및 액션 버튼** 제공

**5. 로그인 API 업데이트** ✅
- 로그인 응답에 `company_role`, `company_size`, `department` 정보 포함
- 회원가입 API에서 해당 필드 처리 추가
- JWT 토큰 페이로드에 역할 정보 포함

**6. 테스트 계정 업데이트** ✅
- **b2b@test.com** 계정에 적절한 권한 부여:
  - `company_role`: `hr_manager`
  - `company_size`: `20_50`
  - `department`: `HR팀`

### 📋 접근 권한 체크 로직

**JavaScript 권한 검증** (workshops.js):
```javascript
function checkAuth() {
  const user = JSON.parse(localStorage.getItem('user'));
  
  // B2B 체크
  if (user.user_type !== 'B2B') return false;
...
1. **회원가입 프로세스 개선**:
   - 회원가입 시 company_role과 company_size 입력 폼 추가
   - B2B 세부 정보 입력 단계 강화
...
3. **원데이 클래스와의 연계**:
   - 소규모 기업을 위한 원데이 클래스 추천
   - 일반 직원을 위한 개인 참가 옵션 안내

---

## 🆕 이전 업데이트 (v1.12.1) - 회원가입 성별 선택 간소화 (2025-11-17)
