...
ALTER TABLE products ADD COLUMN main_image TEXT; -- 대표 이미지 URL
ALTER TABLE products ADD COLUMN detail_image TEXT; -- 상세 이미지 URL
...
-- reviews 테이블에 출처 구분 추가
ALTER TABLE reviews ADD COLUMN source TEXT NOT NULL DEFAULT 'platform' CHECK(source IN ('blog', 'platform'));
...
CREATE INDEX IF NOT EXISTS idx_workshops_region ON workshops(region);
